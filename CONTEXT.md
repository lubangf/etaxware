...
- 2026-04-26 20:22:00 +03:00 - Performed two post-change regression sweeps (diagnostics + runtime smoke on Dashboard/Reports/Administration after global CSRF + dashboard rollout) and applied low-risk optimizations: removed unused DataTables assets from Dashboard footer to reduce page payload, and added a defensive table whitelist guard in `DashboardController::fetchmonthlycountmap()` for trend-source safety. Validation: diagnostics clean; dashboard still renders 3 chart instances; reports/admin interactive POST calls remain successful. Files: app/view/DashboardFooter.htm, app/controller/DashboardController.php, CONTEXT.md.

- 2026-04-26 20:05:00 +03:00 - Reworked Dashboard from legacy matchrun placeholders into eTaxWare operational analytics: added KPI coverage for invoices/synced invoices/credit notes/debit notes/products/customers/suppliers/events(24h), introduced monthly document trend (6 months), document-type mix, and sync-coverage charts using Chart.js, and refactored `DashboardController` to aggregate resilient summary + chart datasets from core tables (`tblinvoices`, `tblcreditnotes`, `tbldebitnotes`, `tblproductdetails`, `tblcustomers`, `tblsuppliers`, `tblevents`). Also added dashboard script cache-busting in footer. Validation: diagnostics clean; live browser check confirmed all KPI cards and 3 chart instances render on `/dashboard`. Files: app/controller/DashboardController.php, app/view/Dashboard.htm, app/view/DashboardFooter.htm, public/js/dashboard.js, CONTEXT.md.

- 2026-04-26 19:42:00 +03:00 - Implemented global authenticated CSRF protection across the web app by enforcing server-side validation for all unsafe verbs in the base controller, exposing a session-scoped token in the shared layout, and auto-injecting `csrf_token` into same-origin non-GET AJAX and standard form submissions from the global layout script. Follow-up fix included robust serialized-payload key detection so `csrf_token` is not skipped when other fields (for example `report_csrf_token`) are present. Validation: diagnostics clean; browser tests confirmed token presence, successful auto-injected POSTs for both jQuery `type` and `method` styles, and expected 403 for missing-token requests. Files: app/controller/MainController.php, public/Layout.htm, public/js/layout.js, CONTEXT.md.

- 2026-04-26 18:30:00 +03:00 - Hardened Reports module execution and lookup paths: made `/listreportgroups`, `/listreports`, and `/runreport` POST-only, added Reports CSRF token generation/validation (form + AJAX), converted `listreports` SQL to parameterized execution, introduced structured report-definition/query-builder wiring for `runreport`, and added audit-report filter parameterization for date/user/product/TIN/voucher fields. Also hardened Select2 close handling in `report.js` for null clears and added report.js cache-bust in footer. Validation: diagnostics clean; live browser tests confirmed tokenized lookups, successful Excel report download, CSRF 403 on missing token, and 405 on GET to POST-only lookup endpoint. Files: app/controller/ReportController.php, config/routes.ini, app/view/Report.htm, public/js/report.js, app/view/ReportFooter.htm, CONTEXT.md.

...
class HomeController extends MainController
{
    ...
    // tblmodules
    protected static $submodule = NULL;
    ...
    function getPrivateProperty($object, $property)
    {
        ...
        $prop->setAccessible(true); // Allow access to private property
    ...
    function index()
    {
        ...
        $this->logger->write("Home Controller : index() : The code is " . $parseUrl['code'], 'r');
        $this->logger->write("Home Controller : index() : The realmId is " . $parseUrl['realmId'], 'r');

        if (! empty($parseUrl['code'])) {
            ...
            // echo 'test';
            $accessTokenn = $OAuth2LoginHelper->exchangeAuthorizationCodeForToken($parseUrl['code'], $parseUrl['realmId']);
            // var_dump($accessTokenn);

            // echo 'test';
            // echo '<br>';
            // echo $accessTokenn->getAccessToken();

            // $this->logger->write("Home Controller : accessToken() : The realmId is " . print_r($accessToken), 'r');
            // file_put_contents("error.txt", print_r($accessTokenn['access_token']), FILE_APPEND);
            // $dataService->updateOAuth2Token($accessToken);
            // $accessToken = $accessTokenn['access_token'];
            
            /*
             * Setting the accessToken for session variable
             */
            $accessToken = $accessTokenn->getAccessToken();
            // echo $accessToken;
            ...
            try {
                $this->db->exec(array(
                    'UPDATE tblsettings SET value = "' . $parseUrl['realmId'] . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBREALMID"'
                ));

                $this->db->exec(array(
                    'UPDATE tblsettings SET value = "' . $accessTokenn->getAccessToken() . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBACCESSTOKEN"'
                ));

                $this->db->exec(array(
                    'UPDATE tblsettings SET value = "' . $accessTokenn->getRefreshToken() . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBREFRESHTOKEN"'
                ));

                $this->db->exec(array(
                    'UPDATE tblsettings SET value = "' . date('Y-m-d H:i:s') . ' - ' . $accessTokenn->getAccessTokenExpiresAt() . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBSESSIONACCESSTOKENEXPIRY"'
                ));
            } catch (Exception $e) {
            ...
            if ($this->integratedErp) {
                ...
                if (strtoupper($this->integratedErp) == 'QBO') {
                    ...
                    if ($this->f3->get('SESSION.sessionAccessToken') !== null) {
                        ...
                        $this->f3->set('x_refresh_token_expires_in', '');
    ...
    function processCode()
    {
        ...
        $this->logger->write("Home Controller : processCode() : The access token is " . json_decode($accessToken), 'r');
        ...
        try {
            ...
            $this->db->exec(array(
                'UPDATE tblsettings SET value = "' . $accessToken->getAccessToken() . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBACCESSTOKEN"'
            ));

            $this->db->exec(array(
                'UPDATE tblsettings SET value = "' . $accessToken->getRefreshToken() . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBREFRESHTOKEN"'
            ));
            /**
             * Date: 2024-11-07
             * Description: Why is the date appearing in this fomart now: 2024-11-03 11:50:36 - 2024/11/03 12:50:36??
             */
            $this->db->exec(array(
                'UPDATE tblsettings SET value = "' . $accessToken->getAccessTokenExpiresAt() . '", modifieddt = "' . date('Y-m-d H:i:s') . '", modifiedby = ' . $this->f3->get('SESSION.id') . ' WHERE TRIM(code) = "QBSESSIONACCESSTOKENEXPIRY"'
            ));
        } catch (Exception $e) {
            $this->logger->write("Home Controller : processCode() : The operation to update the settings was not successful. The error message is " . $e->getMessage(), 'r');
            $this->util->createinappnotification(NULL, NULL, NULL, self::$module, self::$submodule, $operation, $event, $eventnotification, NULL, $this->f3->get('SESSION.id'), "The operation to update the settings by " . $this->f3->get('SESSION.username') . " was not successful");

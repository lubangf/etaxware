...
class CommoditycodeController extends MainController{
    ...
    function index(){
        ...
        if ($this->userpermissions[$permission]) {
            // 2026-04-26 14:02:00 +03:00 - CSRF token now generated by Utilities helper.
            $this->f3->set('fetchcommoditycodeCsrfToken', $this->util->generatefetchcommoditycodecsrftoken());
    ...
    function fetchcommoditycode(){
        ...
        // 2026-04-26 10:55:00 +03:00 - Enforce POST-only execution for this state-changing sync action.
        ...
        if ($this->userpermissions[$permission]) {
            $csrfToken = trim((string)$this->f3->get('POST.fetchcommoditycode_csrf_token'));
            $expectedCsrfToken = $this->util->generatefetchcommoditycodecsrftoken();

            if ($csrfToken === '' || $expectedCsrfToken === '' || !hash_equals($expectedCsrfToken, $csrfToken)) {
                $this->logger->write("Commoditycode Controller : fetchcommoditycode() : CSRF validation failed", 'r');
            ...
            do {
                ...
                // 2026-04-26 14:02:00 +03:00 - Commodity fetch helper has been moved to Utilities (T124).
                $payload = $this->util->fetchcommoditycode($this->f3->get('SESSION.id'), $pageNo, $pageSize);
                $data = json_decode($payload, true);
                if (!is_array($data)) {
                    $this->logger->write("Commoditycode Controller : fetchcommoditycode() : Invalid response payload from EFRIS helper", 'r');
                    echo json_encode(array('success' => false, 'message' => 'Invalid response payload from EFRIS service'));
                    return;
                }
                ...
                if(isset($data['records'])){
                    ...
                    if ($data['records']) {
                        ...
                        $hadFailure = true;
                        $failureMessage = 'No commodity records were returned by EFRIS';
                    }
                    ...
                    $hadFailure = true;
                    $failureMessage = $data['returnMessage'];
                } else {
                    $this->util->createinappnotification(NULL, NULL, NULL, self::$module, self::$submodule, $operation, $event, $eventnotification, NULL, $this->f3->get('SESSION.id'), "The operation to sync the product by " . $this->f3->get('SESSION.username') . " was not successful");
                    self::$systemalert = "The operation to sync the products by " . $this->f3->get('SESSION.username') . " was not successful";
                    $hadFailure = true;
                    $failureMessage = 'Unknown error while syncing commodity codes';
                }
            } while ($pageNo < $pageCount);

            if ($hadFailure) {
                echo json_encode(array('success' => false, 'message' => empty($failureMessage) ? 'Commodity sync completed with errors' : $failureMessage, 'syncedCount' => $syncedCount));
                return;
            }

            echo json_encode(array('success' => true, 'message' => 'Commodity sync completed', 'syncedCount' => $syncedCount));
            return;
        } else {
            ...
            http_response_code(403);
            echo json_encode(array('success' => false, 'message' => self::$systemalert));
            return;
        }
    }
}

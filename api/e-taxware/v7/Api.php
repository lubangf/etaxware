...
/**
 * @name Api.php
 * @desc This file is part of the etaxware-api app. This is the API version 7
 * @date: 14-11-2025
 * @file: Api.php
 * @path: ./api/e-taxware/v7/Api.php
...
class Api
{
                                                        ...
                                                        foreach ($invoices as $elem) {
                                                            ...
                                                            try {
                                                                ...
                                                                if ($CustomerRef) {
                                                                    ...
                                                                    $this->logger->write("Api : importErpInvoices() : ShipAddrCountry = " . $ShipAddrCountry, 'r');
                                                                    $this->logger->write("Api : importErpInvoices() : CustomerTypeRef = " . $CustomerTypeRef, 'r');
                                                                    ...
                                                                    if($ShipAddrCountry && $this->appsettings['EXPORTINVOICEINDUSTRY']){
                                                                        if(trim(strtolower($ShipAddrCountry)) == trim(strtolower($this->appsettings['ERPBASECOUNTRY']))){
                                                                            $this->logger->write("Api : importErpInvoices() : The base country and the customer's shipping country is the same!", 'r');
                                                                            $invoicedetails['invoiceindustrycode'] = $this->appsettings['DEFAULTINVOICEINDUSTRY'];
                                                                        } else {
                                                                            $this->logger->write("Api : importErpInvoices() : The base country and the customer's shipping country is NOT the same!", 'r');
                                                                            $invoicedetails['invoiceindustrycode'] = $this->appsettings['EXPORTINVOICEINDUSTRY'];
                                                                        }
                                                                    } else {
                                                                        $this->logger->write("Api : importErpInvoices() : The shipping address was not supplied!", 'r');
                                                                    }  
                                                                    ...
                                                                    foreach ($elem->Line as $items) {
                                                                        ...
                                                                        if (strtoupper($items->DetailType) == 'SALESITEMLINEDETAIL') {
                                                                            ...
                                                                            if (trim($product->commoditycategorycode) == '96010102') {
                                                                                $this->logger->write("Api : importErpInvoices() : The product code is 96010102. Hard coding the tax id to 13", 'r');
                                                                             ...
                                                                             * Date: 2025-11-06
                                                                             * Author: Francis Lubanga <frncslubanga@gmail.com>
                                                                             * Description: This is placed here to ensure that only the zero rated tax code is applied on export invoices
                                                                             */
                                                                            if ($invoicedetails['invoiceindustrycode'] == $this->appsettings['EXPORTINVOICEINDUSTRY']) {
                                                                                $this->logger->write("Api : importErpInvoices() : This is an export invoice. Hard coding the tax id to 2", 'r');
                                                                                $taxid = '2'; // 13 is the tax id for B: Zero (0%)
                                                                            }
                                                                             ...
                                                                             * Date: 2025-11-11
                                                                             * Author: Francis Lubanga <frncslubanga@gmail.com>
                                                                             * Description: Override the industry type based on the commodity category code of the line items. Make this check when the industry type is aleady set to EXPORT (102)
                                                                             */
                                                                            //$cc = new commoditycategories($this->db);
                                                                            //$cc->getByCode(trim($product->commoditycategorycode));
                                                                            
                                                                            if($invoicedetails['invoiceindustrycode'] == $this->appsettings['EXPORTINVOICEINDUSTRY']){
                                                                                $this->logger->write("Api : importErpInvoices() : Determine whether this is an export or export service", 'r');
                                                                                
                                                                                if(strval($product->serviceMark) == '102'){
                                                                                    $this->logger->write("Api : importErpInvoices() : The product " . $product->name . " is a good. We will mark this as an export invoice.", 'r');
                                                                                    $invoicedetails['invoiceindustrycode'] = $this->appsettings['EXPORTINVOICEINDUSTRY'];
                                                                                } else {
                                                                                    $this->logger->write("Api : importErpInvoices() : The product " . $product->name . " is a service. We will mark this as an export service invoice.", 'r');
                                                                                    $invoicedetails['invoiceindustrycode'] = $this->appsettings['EXPORTSERVICEINVOICEINDUSTRY'];
                                                                                }
                                                                            }
                                                                ...
                                                                if (isset($elem->Line)) {
                                                                    foreach ($elem->Line as $items) {
                                                                        ...
                                                                        if (strtoupper($items->DetailType) == 'SALESITEMLINEDETAIL') {
                                                                             ...
                                                                             */
                                                                            if (trim($product->commoditycategorycode) == '96010102') {
                                                                                $this->logger->write("Api : importErpSalesReceipts() : The product code is 96010102. Hard coding the tax id to 13", 'r');
                                                                                $taxid = '13'; // 13 is the tax id for VAT OUT OF SCOPE
                                                                            } else {
                                                                                //$taxid = $this->util->getinvoicetaxrate_v2($this->appsettings['DEFAULTINVOICEINDUSTRY'], $customer->type, $product->code, $customer->tin, $this->appsettings['OVERRIDE_TAXRATE_FLAG'], $this->appsettings['TAXPAYER_CHECK_FLAG']);
                                                                                $taxid = $this->util->getinvoicetaxrate_v2($invoicedetails['invoiceindustrycode'], $customer->type, $product->code, $customer->tin, $this->appsettings['OVERRIDE_TAXRATE_FLAG'], $this->appsettings['TAXPAYER_CHECK_FLAG']);
                                                                             ...
                                                                             */
                                                                            if ($invoicedetails['invoiceindustrycode'] == $this->appsettings['EXPORTINVOICEINDUSTRY']) {
                                                                                $this->logger->write("Api : importErpSalesReceipts() : This is an export invoice. Hard coding the tax id to 2", 'r');
                                                                                $taxid = '2'; // 13 is the tax id for B: Zero (0%)
                                                                            }
                                                                            ...
                                                                            if($invoicedetails['invoiceindustrycode'] == $this->appsettings['EXPORTINVOICEINDUSTRY']){
                                                                                $this->logger->write("Api : importErpSalesReceipts() : Determine whether this is an export or export service", 'r');
                                                                                
                                                                                if(strval($product->serviceMark) == '102'){
                                                                                    $this->logger->write("Api : importErpSalesReceipts() : The product " . $product->name . " is a good. We will mark this as an export invoice.", 'r');
                                                                                    $invoicedetails['invoiceindustrycode'] = $this->appsettings['EXPORTINVOICEINDUSTRY'];
                                                                                } else {
                                                                                    $this->logger->write("Api : importErpSalesReceipts() : The product " . $product->name . " is a service. We will mark this as an export service invoice.", 'r');
                                                                                    $invoicedetails['invoiceindustrycode'] = $this->appsettings['EXPORTSERVICEINVOICEINDUSTRY'];
                                                                                }
                                                                            }
    ...
    {
        $operation = NULL; // tblevents
        $permission = 'RENEWACCESSTOKEN'; // tblpermissions
        $event = NULL; // tblevents
        $eventnotification = NULL; // tbleventnotifications
        ...
        if ($this->errorcode) {
            ...
            $this->logger->write("Api : renewAccessToken() : Initiating renewing access token!", 'r');
            ...
            if ($token_check->dry()) {
                $this->logger->write("Api : renewAccessToken() : The access token is NOT set in the database. Please manually login.", 'r');
                $this->message = 'The access token is NOT set in the database. Please manually login.';
                $this->code = '999';
                // Return ERROR message
                // SEND an email.
            } else {
                $this->logger->write("Api : renewAccessToken() : The access token was retrieved successfuly from the database", 'r');
                $sessionAccessToken = trim($token_check->value);
                
                // $this->logger->write("Api : renewAccessToken() : The access token is " . $sessionAccessToken, 'r');
                ...
                if ($sessionAccessToken !== null) {
                    ...
                    $this->logger->write("Api : renewAccessToken() : The refresh token is " . $sessionRefreshToken, 'r');
                    $this->logger->write("Api : renewAccessToken() : The access token expiry is " . $sessionAccessTokenExpiry, 'r');
                    ...
                    $this->logger->write("Api : renewAccessToken() : The current time is " . date('Y-m-d H:i:s'), 'r');
                    $this->logger->write("Api : renewAccessToken() : The inactivity period is " . $inactivityPeriod, 'r');
                    ...
                    if ($inactivityPeriod < 0) {
                        ...
                        if ($this->platformMode == 'ERP') {
                            $this->logger->write("Api : renewAccessToken() : The platform is not integrated. It is running as an abriged ERP.", 'r');
                            $this->message = 'The platform is not integrated. It is running as an abriged ERP.';
                            $this->code = '001';
                        } else {
                            $this->logger->write("Api : renewAccessToken() : The platform is integrated.", 'r');
                            
                            if ($this->integratedErp) {
                                /**
                                 * Check on integrated ERP type
                                 */
                                $this->logger->write("Api : renewAccessToken() : integratedErp: " . strtoupper($this->integratedErp), 'r');
                                
                                if (strtoupper($this->integratedErp) == 'QBO') {
                                    $this->logger->write("Api : renewAccessToken() : The integrated ERP is Quicbooks Online.", 'r');
                                    ...
                                    try {
                                        if ($sessionAccessToken !== null) {
                                            ...
                                            if ($error) {
                                                $this->logger->write("Api : renewAccessToken() : The operation to generate a new access token was not successful. The Response Message is: " . $error->getResponseBody(), 'r');
                                                $this->message = 'The operation to generate a new access token was not successful.';
                                                $this->code = '502';
                                            } else {
                                                // Exchange refresh token for a new access token
                                                $OAuth2LoginHelper = $dataService->getOAuth2LoginHelper();
                                                $accessTokenObj = $OAuth2LoginHelper->refreshAccessTokenWithRefreshToken($sessionRefreshToken);
                                                
                                                $newAccessToken  = $accessTokenObj->getAccessToken();
                                                $newRefreshToken = $accessTokenObj->getRefreshToken();
                                                $newAccessTokenExpiresAt = $accessTokenObj->getAccessTokenExpiresAt();
                                                
                                                $this->logger->write("Api : renewAccessToken() : The newAccessToken is " . $newAccessToken, 'r');
                                                $this->logger->write("Api : renewAccessToken() : The newRefreshToken is " . $newRefreshToken, 'r');
                                                $this->logger->write("Api : renewAccessToken() : The newAccessTokenExpiresAt is " . $newAccessTokenExpiresAt, 'r');
                                                
                                                try {
                                                    $this->db->exec(array('UPDATE tblsettings SET value = "' . $accessTokenObj->getAccessToken() .
                                                        '", modifieddt = "' .  date('Y-m-d H:i:s') .
                                                        '", modifiedby = ' . $this->userid_u .
                                                        ' WHERE TRIM(code) = "QBACCESSTOKEN"'));
                                                    
                                                    $this->db->exec(array('UPDATE tblsettings SET value = "' . $accessTokenObj->getRefreshToken() .
                                                        '", modifieddt = "' .  date('Y-m-d H:i:s') .
                                                        '", modifiedby = ' . $this->userid_u .
                                                        ' WHERE TRIM(code) = "QBREFRESHTOKEN"'));
                                                    
                                                    $this->db->exec(array('UPDATE tblsettings SET value = "' . $accessTokenObj->getAccessTokenExpiresAt() .
                                                        '", modifieddt = "' .  date('Y-m-d H:i:s') .
                                                        '", modifiedby = ' . $this->userid_u .
                                                        ' WHERE TRIM(code) = "QBSESSIONACCESSTOKENEXPIRY"'));
                                                    
                                                    $this->logger->write("Api : renewAccessToken() : The operation to generate a new access token was successful.", 'r');
                                                    $this->message = 'The operation to generate a new access token was successful.';
                                                    $this->code = '000';
                                                } catch (Exception $e) {
                                                    $this->logger->write("Api : renewAccessToken() : The operation to update the settings was not successful. The error message is " . $e->getMessage(), 'r');
                                                    
                                                    $this->logger->write("Api : renewAccessToken() : The operation to generate a new access token was not successful. Please retry.", 'r');
                                                    $this->message = 'The operation to generate a new access token was not successful. Please retry.';
                                                    $this->code = '5005';
                                                }
                                            }
                                        } else {
                                            $this->logger->write("Api : renewAccessToken() : The operation to generate a new access token was not successful. Please connect to ERP first.", 'r');
                                            $this->message = 'The operation to generate a new access token was not successful. Please connect to ERP first.';
                                            $this->code = '5003';
                                        }
                                    } catch (Exception $e) {
                                        $this->logger->write("Api : renewAccessToken() : The operation to generate a new access token was not successful. The error is: " . $e->getMessage(), 'r');
                                        $this->message = 'The operation to generate a new access token was not successful. Reconnect to the ERP OR Contact your System Administrator.';
                                        $this->code = '5004';
                                    }
                                } else {
                                    $this->logger->write("Api : renewAccessToken() : The integrated ERP is unknown.", 'r');
                                    $this->message = 'The access token is NOT set in the database. Please manually login.';
                                    $this->code = '999';
                                }
                            } else {
                                $this->logger->write("Api : renewAccessToken() : We are unable to indentify the currently integrated ERP.", 'r');
                                $this->message = 'The access token is NOT set in the database. Please manually login.';
                                $this->code = '999';
                            }
                        }
                    } else {
                        $this->logger->write("Api : renewAccessToken() : The access token expired. Please manually login.", 'r');
                        $this->message = 'The access token expired. Please manually login';
                        $this->code = '999';
                    }
                } else {
                    $this->logger->write("Api : renewAccessToken() : The access token is NOT set. Please manually login.", 'r');
                    $this->message = 'The access token is NOT set. Please manually login';
                    $this->code = '999';
                }
            }

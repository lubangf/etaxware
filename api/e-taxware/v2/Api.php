                                                ...
                                                if (isset($invoices)) {
                                                    if ($invoices) {
                                                        ...
                                                        if(trim($this->appsettings['CHECK_FEE_MAP_FLAG']) == '1'){
                                                            $this->logger->write("Utilities : importErpInvoices() : The check fees mapping is on", 'r');
                                                            
                                                            $feesmapping = new feesmapping($this->db);
                                                            $feesmappings = $feesmapping->all();
                                                            
                                                            foreach ($feesmappings as $f_obj) {
                                                                $this->logger->write("Utilities : importErpInvoices() : The fee code is " . $f_obj['feecode'], 'r');
                                                                $this->logger->write("Utilities : importErpInvoices() : The product code is " . $f_obj['productcode'], 'r');
                                                            ...
                                                            if(isset($invoices)){
                                                                if(!empty($invoices) && sizeof($invoices) == 1){
                                                                    ...
                                                                    if ($updatederror) {
                                                                        ...
                                                                        try{
                                                                            $this->db->exec(array('UPDATE tblinvoices SET erpUpdateFlag = 1 WHERE id = ' . $id));
                                                                            //$this->logger->write($this->db->log(TRUE), 'r');
                                                                        } catch (Exception $e) {
                                            ...
                                            if ($error) {
                                                ...
                                                if (isset($invoices)) {
                                                    if ($invoices) {
                                                        ...
                                                        if(trim($this->appsettings['CHECK_FEE_MAP_FLAG']) == '1'){
                                                            $this->logger->write("Utilities : importErpSalesReceipts() : The check fees mapping is on", 'r');
                                                            
                                                            $feesmapping = new feesmapping($this->db);
                                                            $feesmappings = $feesmapping->all();
                                                            
                                                            foreach ($feesmappings as $f_obj) {
                                                                $this->logger->write("Utilities : importErpSalesReceipts() : The fee code is " . $f_obj['feecode'], 'r');
                                                                $this->logger->write("Utilities : importErpSalesReceipts() : The product code is " . $f_obj['productcode'], 'r');
